...
同一個資料庫另有 `visual_state_events` 資料表，記錄每一次四類視覺狀態切換的
起訖時間與平均信心度，供 Dashboard「分佈摘要」（`GET
/api/analytics/distribution`）彙總今日／本週統計使用。與上述使用者主動儲存的
工作階段不同，這張表**只要 Live Mode 有連線就會自動寫入**，不經過使用者勾選
同意的流程；它不含使用者編號、備註或任何可識別欄位，只有狀態代碼、時間戳與
信心度數字。目前沒有對應的清除／匯出 API，只能直接操作資料庫。
